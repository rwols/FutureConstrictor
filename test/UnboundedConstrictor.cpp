...
TEST_SUITE(UnboundedConstrictorSuite);

TEST(ConstructAndDestruct1) {
  UnboundedConstrictor c;
...
TEST(MapDeletionWhenAll) {
  ...
  m["foo"] = std::make_unique<UnboundedConstrictor>();
  m["bar"] = std::make_unique<UnboundedConstrictor>();
  m["baz"] = nullptr;
  m["qux"] = nullptr;
  for (auto &keyvalue : m) {
